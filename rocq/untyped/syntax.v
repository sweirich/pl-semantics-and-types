...
Lemma congr_abs {m_Val : nat} {s0 : Tm (S (S m_Val))} {t0 : Tm (S (S m_Val))}
...
(xi_Val : fin m_Val -> fin n_Val) (s : Val m_Val) {struct s} : Val n_Val :=
  ...
  | unit _ => unit n_Val
  | abs _ s0 => abs n_Val (ren_Tm (upRen_Val_Val (upRen_Val_Val xi_Val)) s0)
  | zero _ => zero n_Val
...
:=
  ...
  | unit _ => unit n_Val
  | abs _ s0 => abs n_Val (subst_Tm (up_Val_Val (up_Val_Val sigma_Val)) s0)
  | zero _ => zero n_Val
