...
with Tm (n_Val : nat) : Type :=
  ...
  | try : Tm n_Val -> nat -> Tm n_Val -> Tm n_Val.
...
Lemma congr_try {m_Val : nat} {s0 : Tm m_Val} {s1 : nat} {s2 : Tm m_Val}
  {t0 : Tm m_Val} {t1 : nat} {t2 : Tm m_Val} (H0 : s0 = t0) (H1 : s1 = t1)
  (H2 : s2 = t2) : try m_Val s0 s1 s2 = try m_Val t0 t1 t2.
Proof.
exact (eq_trans
         (eq_trans (eq_trans eq_refl (ap (fun x => try m_Val x s1 s2) H0))
            (ap (fun x => try m_Val t0 x s2) H1))
         (ap (fun x => try m_Val t0 t1 x) H2)).
Qed.
...
(s : Tm m_Val) {struct s} : Tm n_Val :=
  ...
  | try _ s0 s1 s2 => try n_Val (ren_Tm xi_Val s0) s1 (ren_Tm xi_Val s2)
  end.
...
(sigma_Val : fin m_Val -> Val n_Val) (s : Tm m_Val) {struct s} : Tm n_Val :=
  ...
  | try _ s0 s1 s2 =>
      try n_Val (subst_Tm sigma_Val s0) s1 (subst_Tm sigma_Val s2)
  end.
...
subst_Tm sigma_Val s = s :=
  ...
  | try _ s0 s1 s2 =>
      congr_try (idSubst_Tm sigma_Val Eq_Val s0) (eq_refl s1)
        (idSubst_Tm sigma_Val Eq_Val s2)
  end.
...
ren_Tm xi_Val s = ren_Tm zeta_Val s :=
  ...
  | try _ s0 s1 s2 =>
      congr_try (extRen_Tm xi_Val zeta_Val Eq_Val s0) (eq_refl s1)
        (extRen_Tm xi_Val zeta_Val Eq_Val s2)
  end.
...
subst_Tm sigma_Val s = subst_Tm tau_Val s :=
  ...
  | try _ s0 s1 s2 =>
      congr_try (ext_Tm sigma_Val tau_Val Eq_Val s0) (eq_refl s1)
        (ext_Tm sigma_Val tau_Val Eq_Val s2)
  end.
...
{struct s} : ren_Tm zeta_Val (ren_Tm xi_Val s) = ren_Tm rho_Val s :=
  ...
  | try _ s0 s1 s2 =>
      congr_try (compRenRen_Tm xi_Val zeta_Val rho_Val Eq_Val s0)
        (eq_refl s1) (compRenRen_Tm xi_Val zeta_Val rho_Val Eq_Val s2)
  end.
...
{struct s} : subst_Tm tau_Val (ren_Tm xi_Val s) = subst_Tm theta_Val s :=
  ...
  | try _ s0 s1 s2 =>
      congr_try (compRenSubst_Tm xi_Val tau_Val theta_Val Eq_Val s0)
        (eq_refl s1) (compRenSubst_Tm xi_Val tau_Val theta_Val Eq_Val s2)
  end.
...
ren_Tm zeta_Val (subst_Tm sigma_Val s) = subst_Tm theta_Val s :=
  ...
  | try _ s0 s1 s2 =>
      congr_try (compSubstRen_Tm sigma_Val zeta_Val theta_Val Eq_Val s0)
        (eq_refl s1) (compSubstRen_Tm sigma_Val zeta_Val theta_Val Eq_Val s2)
  end.
...
subst_Tm tau_Val (subst_Tm sigma_Val s) = subst_Tm theta_Val s :=
  ...
  | try _ s0 s1 s2 =>
      congr_try (compSubstSubst_Tm sigma_Val tau_Val theta_Val Eq_Val s0)
        (eq_refl s1)
        (compSubstSubst_Tm sigma_Val tau_Val theta_Val Eq_Val s2)
  end.
...
(s : Tm m_Val) {struct s} : ren_Tm xi_Val s = subst_Tm sigma_Val s :=
  ...
  | try _ s0 s1 s2 =>
      congr_try (rinst_inst_Tm xi_Val sigma_Val Eq_Val s0) (eq_refl s1)
        (rinst_inst_Tm xi_Val sigma_Val Eq_Val s2)
  end.
...
Inductive Frame (n_Val : nat) : Type :=
  | f_try : nat -> Tm n_Val -> Frame n_Val
  | f_let : Tm (S n_Val) -> Frame n_Val.

Lemma congr_f_try {m_Val : nat} {s0 : nat} {s1 : Tm m_Val} {t0 : nat}
  {t1 : Tm m_Val} (H0 : s0 = t0) (H1 : s1 = t1) :
  f_try m_Val s0 s1 = f_try m_Val t0 t1.
Proof.
exact (eq_trans (eq_trans eq_refl (ap (fun x => f_try m_Val x s1) H0))
         (ap (fun x => f_try m_Val t0 x) H1)).
Qed.
...
Definition subst_Frame {m_Val : nat} {n_Val : nat}
  (sigma_Val : fin m_Val -> Val n_Val) (s : Frame m_Val) : Frame n_Val :=
  match s with
  | f_try _ s0 s1 => f_try n_Val s0 (subst_Tm sigma_Val s1)
  | f_let _ s0 => f_let n_Val (subst_Tm (up_Val_Val sigma_Val) s0)
  end.

Definition idSubst_Frame {m_Val : nat} (sigma_Val : fin m_Val -> Val m_Val)
  (Eq_Val : forall x, sigma_Val x = var m_Val x) (s : Frame m_Val) :
  subst_Frame sigma_Val s = s :=
  match s with
  | f_try _ s0 s1 =>
      congr_f_try (eq_refl s0) (idSubst_Tm sigma_Val Eq_Val s1)
  | f_let _ s0 =>
      congr_f_let
        (idSubst_Tm (up_Val_Val sigma_Val) (upId_Val_Val _ Eq_Val) s0)
  end.

Definition ext_Frame {m_Val : nat} {n_Val : nat}
  (sigma_Val : fin m_Val -> Val n_Val) (tau_Val : fin m_Val -> Val n_Val)
  (Eq_Val : forall x, sigma_Val x = tau_Val x) (s : Frame m_Val) :
  subst_Frame sigma_Val s = subst_Frame tau_Val s :=
  match s with
  | f_try _ s0 s1 =>
      congr_f_try (eq_refl s0) (ext_Tm sigma_Val tau_Val Eq_Val s1)
  | f_let _ s0 =>
      congr_f_let
        (ext_Tm (up_Val_Val sigma_Val) (up_Val_Val tau_Val)
           (upExt_Val_Val _ _ Eq_Val) s0)
...
Definition compSubstSubst_Frame {k_Val : nat} {l_Val : nat} {m_Val : nat}
  (sigma_Val : fin m_Val -> Val k_Val) (tau_Val : fin k_Val -> Val l_Val)
  (theta_Val : fin m_Val -> Val l_Val)
  (Eq_Val : forall x, funcomp (subst_Val tau_Val) sigma_Val x = theta_Val x)
  (s : Frame m_Val) :
...
Ltac asimpl' := repeat (first
                 ...
                 | progress
                    cbn[subst_Frame subst_Tm subst_Val ren_Tm ren_Val]
                 | progress fsimpl ]).
...
Ltac asimpl := check_no_evars;
                repeat
                 unfold VarInstance_Val, Var, ids, Ren_Val, Ren1, ren1,
                  ...
                  Subst1, subst1, Subst_Tm, Subst1, subst1, Subst_Frame,
                  Subst1, subst1 in *;
                asimpl'; minimize.
