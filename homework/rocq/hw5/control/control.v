...
Inductive primitive : Tm 0 -> Tm 0 -> Prop := 
  ...
  | s_unfold : forall v : Val 0, 
      unfold (fold v) ~>> ret v 
  (* NEW: primitive step rules for lists *)
  | s_app_zero v1 v2 : 
      app (v_cons v1 v2) zero ~>> ret v1
  | s_app_succ v1 v2 k : 
      app (v_cons v1 v2) (succ k) ~>> (app v2 k)
  | s_app_nil v : 
      app v_nil v ~>> raise (exn 0)
...
Inductive step : machine -> machine -> Prop := 
  ...
  | s_continue s1 s2 v :
    ...
    (* invoke an effect handler, raising an exception *)
  | s_discontinue s s' v : 
    step (s, discontinue (cont s') v) (s', raise v)
  
  (* TODO: add step rule for exit *)
...
Inductive typing_val  {n} (Γ : Ctx n) : Val n -> Ty 0 -> Prop := 
  ...
  (** lists *)
  | t_nil τ : 
    typing_val Γ v_nil (List τ)

  | t_cons τ v1 v2 : 
    typing_val Γ v1 τ -> 
    typing_val Γ v2 (List τ) -> 
    typing_val Γ (v_cons v1 v2) (List τ)
...
with typing {n} (Γ : Ctx n) : Tm n -> Ty 0 -> Prop := 
  ...
  (* lists *)
  | t_nth v1 v2 τ : 
    typing_val Γ v1 (List τ) -> 
    typing_val Γ v2 Nat ->
    typing Γ (app v1 v2) τ

  (* TODO: add typing rules for exit and discontinue *)
