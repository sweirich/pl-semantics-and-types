...
Lemma semantic_zero {n} {Γ : Ctx n} {k:nat} :
   Γ ⊨v zero ∈ Nat.
Proof.
  intros ρ ρH.
  cbn. 
...
Lemma semantic_ifz {n} {Γ : Ctx n} v {e0:Tm n}{e1: Tm (S n)}{τ}  : 
  ...
  Γ ⊨ ifz v e0 e1 ∈ τ.
Proof.
  intros IHv IHe0 IHe1.
  intros ρ ρH.
  asimpl.
  specialize (IHv ρ ρH).
  (* NB: destructing the scrutinee is subtle. The notation v[ρ] in a proof stands for a 
     generic substitution through a type class instance. But, Autosubst prints
     specific substitution (i.e. subst_Val) with the same notation. So we need 
     to simplify the notation and then rewrite with it. *)
  destruct (v[ρ]) eqn:EQ. 
  all: asimpl in EQ; rewrite EQ; rewrite EQ in IHv; cbn in IHv; try done.
  - eapply reverse_evaluation with (e' := e0[ρ]); eauto with rec.
  - eapply reverse_evaluation.
    eapply Small.s_ifz_succ.
    asimpl. 
    eapply IHe1. 
...
Lemma semantic_ret {n} (Γ : Ctx n) v τ : 
  Γ ⊨v v ∈ τ ->
  Γ ⊨ ret v ∈ τ.
Proof.
(* FILL IN HERE *) Admitted.

Lemma semantic_let {n} (Γ : Ctx n) e1 e2  τ1 τ2 : 
  Γ ⊨ e1 ∈ τ1 ->
  τ1 .: Γ ⊨ e2 ∈ τ2 -> 
  Γ ⊨ let_ e1 e2 ∈ τ2.
Proof.
